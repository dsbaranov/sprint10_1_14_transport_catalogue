...
void RequestHandler::AddBusses(const std::vector<BusRequest> &requests) {
  ...
  for (const BusRequest &request : requests) {
    ...
    if (!request.is_roundtrip) {
      stops.insert(stops.end(), std::next(bus_stops.rbegin()),
                   bus_stops.rend());
    }

    Bus new_bus{request.name, stops};
...
Response RequestHandler::GetStatistics(const StatRequest &request) const {
  if (request.type == "Stop"s) {
    ...
    const std::set<std::string_view> &buses =
        catalogue_.FindBusesByStop(stop_ptr);
    ...
    BusStatistics statistics = catalogue_.GetBusStatistics(bus_ptr);

    return BusResponse{request.id, static_cast<int>(statistics.stops),
                       static_cast<int>(statistics.unique_stops),
                       statistics.distance_statistics.distance,
                       statistics.distance_statistics.curvature};
  }
  return ErrorResponse(request.id);
}